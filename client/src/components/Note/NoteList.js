...
class  NoteList extends React.Component{
    ...
    // findCategory = (id) => {
    //     return this.props.category.filter(category=>category._id == id)
    // }
                            ...
                            return (
                                <Col md="3" key={index}>
                                    <Card body inverse style={{ backgroundColor: '#333', borderColor: '#333' }} className="mb-4">
                                        <CardTitle><h4>{index+1}:  {note.title}</h4></CardTitle>
                                        
                                        <CardText>Description: {note.body}</CardText>


                                        <CardText> Category: {note.category?note.category.name : 'N.A'}</CardText>
